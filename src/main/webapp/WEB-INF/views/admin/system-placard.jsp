...
<section class="Hui-article-box">
    ...
    <div class="Hui-article">
        <article class="cl pd-20">
            <%--TODO 添加form表单--%>
            <div>
                <img src="${pageContext.request.contextPath}/images/moren.jpg" alt="" id="showPlacard">
                <br>
                <%--限制只能选择图片--%>
                <input type="file" id="placard" name="file" value="选择海报" accept="image/gif,image/png,image/jpeg,image/jpg" >
            </div>
        </article>
    </div>
</section>
...
<!--请在下方写此页面业务相关的脚本-->
<script>
    $("#placard").change(function () {
        var objUrl = getObjectURL(this.files[0]);
        console.log("objUrl = " + objUrl);
        if (objUrl) {
            $("#showPlacard").attr("src", objUrl);
        }
    });
    //建立一個可存取到該file的url
